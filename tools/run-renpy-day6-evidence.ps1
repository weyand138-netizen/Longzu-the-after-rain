[CmdletBinding()]
param(
    [string]$EvidenceRoot = "production\qa\evidence\day6-content-validation-2026-08-13-verified",
...
while (-not $process.HasExited -and -not $statusLine) {
    ...
    if ([DateTime]::UtcNow -ge $deadline) { $process.Kill(); $process.WaitForExit(5000) | Out-Null; throw "Ren'Py Day 6 evidence run timed out after $TimeoutSeconds seconds." }
    Start-Sleep -Milliseconds 100
}
if (-not $statusLine -and $process.HasExited) { throw "Ren'Py Day 6 evidence run exited before emitting a terminal testcase status." }
...
$result = [ordered]@{
    ...
    project = $repo
    day6_source_sha256 = (Get-FileHash -LiteralPath (Join-Path $repo "game\chapters\day6.rpy") -Algorithm SHA256).Hash.ToLowerInvariant()
...
$captures = @(
    "visual\day6_cost_1280x720_keyboard_silent_reduced_motion.png",
    "visual\day6_commitment_1280x720_keyboard_silent_reduced_motion.png",
    "visual\day6_cost_1280x720_font_1_5_high_contrast.png",
    "visual\day6_commitment_1280x720_font_1_5_high_contrast.png"
)
foreach ($relativeCapture in $captures) {
    $sourceCapture = Join-Path $repo (Join-Path "tests\screenshots" $relativeCapture)
    if (-not (Test-Path -LiteralPath $sourceCapture -PathType Leaf)) { throw "Required Day 6 capture was not produced: $sourceCapture" }
