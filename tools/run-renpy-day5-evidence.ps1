[CmdletBinding()]
param(
    [string]$EvidenceRoot = "production\qa\evidence\day5-content-validation-2026-08-13-reviewed-verified",
...
while (-not $process.HasExited -and -not $statusLine) {
    ...
    if ([DateTime]::UtcNow -ge $deadline) {
        ...
        throw "Ren'Py Day 5 evidence run timed out after $TimeoutSeconds seconds."
    }
    Start-Sleep -Milliseconds 100
}

if (-not $statusLine -and $process.HasExited) {
    throw "Ren'Py Day 5 evidence run exited before emitting a terminal testcase status."
}
...
$result = [ordered]@{
    ...
    python = $python
    project = $repo
    day5_source_sha256 = (Get-FileHash -LiteralPath (Join-Path $repo "game\chapters\day5.rpy") -Algorithm SHA256).Hash.ToLowerInvariant()
...
$captures = @(
    "visual\day5_truth_1280x720_keyboard_silent_reduced_motion.png",
    "visual\day5_response_1280x720_keyboard_silent_reduced_motion.png",
    "visual\day5_truth_1280x720_font_1_5_high_contrast.png",
    "visual\day5_response_1280x720_font_1_5_high_contrast.png"
)
foreach ($relativeCapture in $captures) {
    $sourceCapture = Join-Path $repo (Join-Path "tests\screenshots" $relativeCapture)
    if (-not (Test-Path -LiteralPath $sourceCapture -PathType Leaf)) { throw "Required Day 5 capture was not produced: $sourceCapture" }
